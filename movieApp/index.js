const API_key="50ff75a4";
const searchInput=document.getElementById("searchInput");
const results=document.getElementById("results");
const loadMoreBtm=document.getElementById("loadMoreBtn");
...
searchInput.addEventListener("input",()=>{
    debounce(async ()=>{
        ...
        renderMovies(movies);
        loadMoreBtm.style.display=movies.length>=10?"block":"none";
    })
})

loadMoreBtm.addEventListener("click",async()=>{
    currentPage++;
    const movies=await fetchMovies(currentQuery,currentPage);
    renderMovies(movies,true);
    if(movies.length<10) loadMoreBtm.style.display="none";
})
